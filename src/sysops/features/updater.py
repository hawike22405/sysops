...
def _repo_root() -> Path:
    """Locate the retained source checkout used by the official installer."""
...
def check_for_update() -> tuple[bool, str]:
    """Fetch the remote branch and report whether an update is available."""
    try:
        repo = _repo_root()
...
def run_update() -> None:
    """Check for updates, schedule the detached installer, then exit."""
    ...
    helper = (
        _write_windows_helper(pid, repo, python_exe)
        if os.name == "nt"
        else _write_posix_helper(pid, repo, python_exe)
    )
